...
class figlist():
    ...
    def switch(self,name,**kwargs):
        ...
        if name in self.figurelist:
            ...
            raise ValueError("The plot "+name+" does not already exist in "+', '.join(self.figurelist))
        return gca()
    def new(self,name,**kwargs):
        if self.verbose: print lsafe('DEBUG figurelist, called with',name)
        if name in self.figurelist:
            raise ValueError("The plot "+name+" already exists in"+', '.join(self.figurelist))
        else:
            fig = figure(len(self.figurelist)+1,**kwargs)
            fig.add_subplot(111)
            if self.verbose: print lsafen('added, figure',len(self.figurelist)+1,'because not in figurelist',self.figurelist)
...
class nddata (object):
    ...
    def run_nopop(self,func,axis):
        ...
        return self
    #}}}
    def splice_in(self,axisname,arg,startingindex,throw_away = False):
        r'''splice stuff to add into this array, starting at startingindex -- which can be either an integer (for index) or float (for axis position) or function (to define a region of the axis to splice in)
        right now, this is only tested + works for 1D stuff --> for higher dimensions, should work but not tested'''
        # Example code:
        # a = nddata(r_[0:10.],[-1],['t']).labels('t',r_[0:10.]/10)
        # b = nddata(r_[0:10.]*10,[-1],['t']).labels('t',r_[0:10.]/10)
        # c = b['t',lambda x: x>=0.5]
        # a.splice_in('t',c,0.5)
        #
        # a = nddata(r_[0:10.],[-1],['t']).labels('t',r_[0:10.]/10-0.5)
        # b = nddata(r_[0:10.]*10,[-1],['t']).labels('t',r_[0:10.]/10-0.5)
        # a.splice_in('t',b,lambda x: abs(x)<0.25)
        taxis = self.getaxis(axisname)
        if (shape(arg.data)!=shape(self.data)):
            self.matchdims(arg)
            arg.matchdims(self)
        if taxis is None:
            dt1 = None
            ...
            dt1 = taxis[1] - taxis[0]
        if arg.getaxis(axisname) is None:
            dt2 = None
        else:
            dt2 = arg.getaxis(axisname)[1] - arg.getaxis(axisname)[0]
        testf = lambda x: x+1
        if abs(dt1 - dt2)>1e-16:
            raise ValueError('The resolution of this array ('+repr(dt1)+') does not match the resolution of the stuff to add ('+repr(dt2)+")+ so I'm going to refuse to do this!!!")
        if type(startingindex) in [double,float,float64,float32]:
            startingindex = abs(taxis - startingindex).argmin()
        elif type(startingindex) in [int,int32,int64]:
            pass
        elif type(startingindex) == type(testf):
            self[axisname,startingindex] = arg[axisname,startingindex]
            return self
        else:
            raise ValueError("splice_in doesn't understand the type of your startingindex"+type(startingindex))
        axis_number = self.axn(axisname)
        thisindex = [slice(None,None,None)]*len(self.dimlabels)
        stoppingindex = startingindex+ndshape(arg)[axisname]
        if throw_away == True:
            if stoppingindex-startingindex > ndshape(self)[axisname]:
                stoppingindex = startingindex + ndshape(self)[axisname]
        thisindex[axis_number] = slice(startingindex,stoppingindex,None)
        try:
            self.data[tuple(thisindex)] = arg.data
        except:
            raise CustomError("I'm having trouble splicing the data you gave me, which is of shape",arg.data.shape,"into this array, which has shape",self.data.shape,"and I slice with",tuple(thisindex),"to get something of shape",self.data[tuple(thisindex)].shape)
        return self
    ...
    def convolve(self,axisname,filterwidth,convfunc = (lambda x,y: exp(-(x**2)/(2.0*(y**2))))):
        ...
        x = x-x.mean()
        if log(len(x))/log(2) != 0:
            print "convolve warning --> your input data is not a power of 2 long!"
        newshape[thisaxis] = len(x)
        x.reshape(newshape)
        #}}}
        myfilter = convfunc(x,filterwidth)
        myfilter /= myfilter.sum()
        print 'DEBUG CONVOLVE: I am about to run a convolutions with filter length',myfilter.shape,'and data length',self.data.shape
        self.data = fftconvolve(self.data,myfilter,mode='same') # I need this, so the noise doesn't break up my blocks
        print 'DEBUG CONVOLVE -- done'
        return self
    def _ft_conj(self,x):
        pairs = [('s','Hz'),('m',r'm^{-1}')]
        a,b = zip(*tuple(pairs))
    ...
    def ft(self,axes,shiftornot=False,shift=None,pad = False):
        ...
        for j in range(0,len(axes)):
            ...
            #if log(padded_length)/log(2) != 0:
            #    print "ft warning --> your input data is not a power of 2 long!"
            #if log(padded_length)/log(2) != 0:
            #    print "ft warning --> your input data is not a power of 2 long!"
            if pad is True:
    ...
    def __getitem__(self,args):
        ...
        if len(self.axis_coords)>0:
            ...
            return nddata(self.data[indexlist],
                    ...
                    axis_coords_units = self.axis_coords_units,
                    data_units = self.data_units,
                    data_error = newerror)
        else:
            return nddata(self.data[indexlist],self.data[indexlist].shape,newlabels,axis_coords_units = self.axis_coords_units,data_units = self.data_units)
