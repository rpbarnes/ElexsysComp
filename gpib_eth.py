#from matlablike import * # for now because open suse is a pain in the ass and zypper blows because it doesn't have correct repositories.
import os
import re
import socket
import struct
...
class gpib():
    ...
    def ver(self):
        ...
        if versionstring[0:8]=='Prologix':
            ...
            raise RuntimeError('Error! can\'t find prologix on %s:%d'%(ip,sock))
    def close(self):
...
class field_controller ():
    def __init__(self,gpibtalk=4,gpiblisten=36,sock = 1235,ip = '127.0.0.1'):
