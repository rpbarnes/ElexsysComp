...
# the usbnumber on the new computer seems to be 3
...
class gpib():
    def __init__(self,usbnumber,timeout = 5):
        ...
        self.serial.write('++mode 1'+"\r\n")
        waittime = timeout/50.
        if waittime < 0.05: waittime = 0.05
        time.sleep(waittime)
        self.serial.write('++ifc'+"\r\n")
        time.sleep(waittime)
        self.serial.write('++auto 1'+"\r\n")
        time.sleep(waittime)
        #self.serial.write('++eoi 0'+"\r\n")
        #time.sleep(waittime)
        self.caddr = -1
        ...
        #    raise
    def close(self):
        self.serial.write('++clr'+"\r\n")
        self.serial.write('++loc'+"\r\n")
        self.serial.close()
    ...
    def respond(self,addr,gpibstr,printstr = '%s'):
        self.serial.write('++auto 1'+"\r\n") # because sometimes, it's just talking at me nonstop
        ...
        self.serial.write('++auto 0'+"\r\n") # now set back into the normal mode, where I need to request a response
        return retval
...
class gpib_eth():
    ...
    def respond(self,addr,gpibstr,printstr):
        ...
        print printstr % self.readline(addr)
...
def powermeter(comport = DEFAULTCOM,gpibaddressrange = [15]+range(1,21)):
